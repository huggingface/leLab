...
- [datasets.py](lelab/datasets.py) — dataset listing (local cache + Hub, merged under a `source` field), merging local datasets into a new one (`/merge-datasets`, wraps `lerobot.datasets.dataset_tools.merge_datasets`) plus the episode-browsing handlers behind `/dataset-episodes`, `/dataset-episode`, `/dataset-frame`, `/dataset-thumbnails`, `/dataset-motion` and `/dataset-video`.
